      ...
      <div className="container mx-auto flex justify-between items-center px-6">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <Link
              ...
              className={`relative text-lg font-medium transition-all duration-300 group ${activeItem === item.path ? "text-yellow-500" : "text-white hover:text-yellow-500"
                }`}
            >
              {item.name}
              <span className={`absolute -bottom-2 left-0 w-0 h-0.5 bg-yellow-500 transition-all duration-300 ${activeItem === item.path ? "w-full" : "group-hover:w-full"
                }`}></span>
            </Link>
          ))}
        ...
        >
          <div className="w-6 h-6 relative flex items-center justify-center">
            <span
              className={`absolute h-0.5 w-full bg-white transform transition-all duration-300 ${isMenuOpen ? "rotate-45 top-3" : "rotate-0 top-1"
                }`}
            ></span>
            <span
              className={`absolute h-0.5 w-full bg-white transform transition-all duration-300 ${isMenuOpen ? "opacity-0 w-0" : "opacity-100 w-full top-3"
                }`}
            ></span>
            <span
              className={`absolute h-0.5 w-full bg-white transform transition-all duration-300 ${isMenuOpen ? "-rotate-45 top-3" : "rotate-0 top-5"
                }`}
            ></span>
          </div>
        </button>
        ...
        <div
          className={`absolute top-full left-0 w-full bg-gradient-to-b from-gray-900 to-black transition-all duration-500 ease-in-out ${isMenuOpen ? "max-h-96 opacity-100 shadow-xl" : "max-h-0 opacity-0 pointer-events-none"
          ...
          <ul className="flex flex-col items-center space-y-5 py-6">
            {navItems.map((item) => (
              <li key={item.path} className="w-full text-center">
                <Link
                  to={item.path}
                  className={`block py-2 px-4 text-lg font-medium transition-all duration-300 ${activeItem === item.path
